  ...
  return (
    <section className="h-full flex flex-col">
      <header className="h-16 bg-[#f0f2f5] flex items-center px-4 border-b border-gray-200">
        <button onClick={onClose} className="text-[#54656f] mr-4" aria-label="Close group info">
          <svg
      ...
      <div className="p-4 flex flex-col items-center border-b border-gray-200">
        <div className="w-24 h-24 rounded-full bg-gray-300 overflow-hidden mb-4">
          <img
            src={chat.avatar || `/placeholder.svg?height=96&width=96`}
            alt={chat.name}
            className="w-full h-full object-cover"
          />
        </div>
        <h3 className="text-xl font-medium">{chat.name}</h3>
        <p className="text-sm text-gray-500 mt-1">Group · {chat.participants.length} participants</p>
      ...
      <div className="p-4 border-b border-gray-200">
        ...
        <p className="text-sm text-gray-700">{chat.description || "No description provided for this group."}</p>
      </div>

      <div className="flex-1 overflow-y-auto">
        <div className="p-4">
          <h4 className="text-sm font-medium text-[#128C7E] mb-2">{chat.participants.length} participants</h4>
            ...
            {chat.participants.map((participant) => (
              <li key={participant.id} className="flex items-center py-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{participant.isOnline ? "Online" : "Last seen recently"}</p>
                </div>
              </li>
            ))}
          </ul>
